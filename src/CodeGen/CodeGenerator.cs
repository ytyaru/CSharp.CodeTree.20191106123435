...
//            builder.Insert(builder.Length, Indent, indent).Append(node.Value);
        ...
        class BlockCodeGenerator
        {
            ...
            public void Blocking(INode node, int indent) => Block(node, Start, indent);
            public void Blocked(INode node, int indent) => Block(node, End, indent);
            private void Block(INode node, string keyword, int indent)
            {
